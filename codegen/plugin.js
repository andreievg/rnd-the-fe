  ...
  plugin(schema, documents) {
    ...
    for (const doc of documents) {
      for (const def of doc.document.definitions) {
        ...
        out.push("");
        // urql's gql() parses to a TypedDocumentNode; we assert the concrete
        // Result/Variables so the call site is fully typed.
        out.push(
          `export const ${Name} = gql(${JSON.stringify(docText)}) as unknown as ` +
